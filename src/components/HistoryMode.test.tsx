...
const generations: Generation[] = [
  {
    id: 'portrait-9', nodeId: 'kael', createdAt: '2026-08-02T12:00:00Z', preset: 'portrait',
    viewType: null, userPrompt: '', compiledPrompt: 'portrait nine', negativePrompt: '',
    backend: 'comfyui', model: 'flux-dev', seed: 9,
    params: { aspect: '3:4', width: 768, height: 1024 }, outputAssetIds: ['asset-9'],
    influenceSnapshot: { layers: [] },
  },
  {
    id: 'sheet-7', nodeId: 'kael', createdAt: '2026-07-01T12:00:00Z', preset: 'character_sheet',
    viewType: null, userPrompt: '', compiledPrompt: 'sheet seven', negativePrompt: '',
    backend: 'gemini', model: 'gemini-image', seed: 7, params: {}, outputAssetIds: [],
    influenceSnapshot: { layers: [] },
  },
]
  ...
  it('filters immutable receipts by preset and seed and opens their detail', async () => {
    render(
      <QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}>
        <HistoryMode nodes={[summary({ id: 'kael', name: 'Kael' })]} readOnly={false} onJump={() => {}} />
      </QueryClientProvider>,
    )
    expect(await screen.findAllByRole('button', { name: /Open generation/ })).toHaveLength(2)
    fireEvent.change(screen.getByLabelText('Filter history by preset'), { target: { value: 'portrait' } })
