...
export function HistoryMode({
  ...
  const [opened, setOpened] = useState<{ generation: Generation; imageSrc: string | null } | null>(null)
  const names = useMemo(() => new Map(nodes.map((node) => [node.id, node.name])), [nodes])
  const receipts = history.data ?? []
  const presets = useMemo(() => [...new Set(receipts.map((item) => item.preset))].sort(), [receipts])
  ...
  return (
    <main className="history-mode" aria-label="Generation history">
      ...
      <div className="history-filters" aria-label="Generation history filters">
        <label>
          <span>Preset</span>
          <select aria-label="Filter history by preset" value={preset} onChange={(event) => setPreset(event.target.value)}>
        ...
        <label>
          <span>Model</span>
          <select aria-label="Filter history by model" value={model} onChange={(event) => setModel(event.target.value)}>
            <option value="all">All models</option>
            {models.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <label><span>From</span><input aria-label="Filter history from date" type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label>
        <label><span>To</span><input aria-label="Filter history to date" type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label>
        <label><span>Seed</span><input aria-label="Filter history by seed" inputMode="numeric" value={seed} onChange={(event) => setSeed(event.target.value.replace(/\D/g, ''))} placeholder="Any seed" /></label>
      </div>

      {history.isError && <p className="history-error">Could not read generation history: {api.errorMessage(history.error)}</p>}
...
function HistoryTile({
  ...
  return (
    <article className="history-tile">
      <button type="button" className="history-open" onClick={onOpen} aria-label={`Open generation ${generation.id}`}>
        <div className="history-image">
          {thumb.data ? <img src={convertFileSrc(thumb.data)} alt={generation.compiledPrompt} /> : <span>No preview</span>}
        </div>
        <div className="history-tile-copy">
          <b>{nodeName}</b>
          <span>{generation.preset} · {generation.model}</span>
          <span>seed {generation.seed} · {new Date(generation.createdAt).toLocaleString()}</span>
          <p>{generation.compiledPrompt}</p>
        </div>
      </button>
      {onJump && <button className="btn-mini" type="button" onClick={onJump}>Open entity</button>}
    </article>
  )
}
