...
3. **Choose the options.** **Faces** (3,000 to 1,500,000, default 500,000), a reconstruction
   **Mode**, and **PBR materials** where the provider offers them. A line beneath states exactly
   what is about to be sent: the model, how many views, and which ones.
4. **Reconstruct.** Press **Reconstruct mesh**. On a paid provider you first tick a consent box that
...
5. **Inspect and export.** The finished mesh appears in the viewer beside the sheet — an inline
   three.js turntable with a **Wireframe** toggle and an orange one-metre marker — with **Show in
folder** GLB** and **Export copy…** in the same toolbar.
...
> let you start until every view the selected provider needs is on screen and chosen. The front view
...
Turnaround is not a generic "spin the character" preset. It exists to feed a specific 3D provider
whose headline capability is eight-view reconstruction, so it emits exactly the view types that
provider names, in the order it wants them:
...
The mesh provider accepts PNG and JPEG only, wants a minimum side of 128px and a maximum of 5000px,
...
The provider protocol is asynchronous — submit, then poll — and a reconstruction is a job in the
same queue as everything else, with the same Stop button and the same progress line. That queue runs
...
The hosted provider charges per submitted job — including one you cancel while it is running — and
...
You pick the 3D provider in Settings, per project. Local ComfyUI mesh generation is an explicit
...
> the selected provider takes fewer views than a turnaround has, the panel says which ones it will
> actually send and marks the rest `not sent to this provider` rather than hiding them.
>
