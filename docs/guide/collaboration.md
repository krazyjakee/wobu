...
Two ways to work together: several people on one folder, or several people holding copies that
reconcile directly between machines. Neither involves a server, an account, or infrastructure Wobu
runs on your behalf.
...
| Node Markdown | Edited continuously | **The only real surface** |
...
dot on the navigator row, a session count in the status bar, and a banner in the editor of a node
somebody else has open. A session whose heartbeat goes stale for a minute is treated as dead.
...
*their* version — and three decisions: **Keep mine**, **Keep theirs**, or **Open both**, plus
**Reveal** to open the folder. Identical runs of lines are collapsed, and if the two files turn out
...
- **The index absorbs it.** After the first open, the workspace renders entirely from the local
  index; the folder is only touched for files that actually changed.
...
> **Both peers must be online at the same time** There is no always-on seed node. A collaborator's
> offline edits land the next time you are both running Wobu — the same behaviour as Syncthing or
> Resilio. The status bar says so in as many words: peer edits arrive only while both people run
> Wobu.

### How it reconciles

No CRDTs, no vector clocks, and **no change to the on-disk format** — files stay hand-editable and
Obsidian keeps working. Wobu already stores a content hash per node; sync additionally remembers,
...
- **Deletes do not propagate.** There are no tombstones, so a file you have and a peer does not is
  ...
  absence is the one that cannot destroy work. Deleting a node on your copy does not delete it on
  theirs.
...
switches off everything that writes: creating, renaming, moving, deleting and duplicating nodes,
...
| --- | --- |
| Share unmounts mid-session | Detects it, shows a banner, retries with backoff, holds writes — and keeps the whole interface readable from the local index. A held save resends when the share returns. |
| Read-only share | Detected on open, as above. |
| Two people generate for the same node | No conflict at all. Separate ULID-named records; both results appear. |
| A sync client mangles a file | The file is listed at the top of the navigator with the parser's own error and a **Reveal** button. It is never silently skipped and never overwritten. |
...
Real-time co-editing, operational transforms, a server component and per-node hard locks are all
