...
│    │ ▾ Species          │ │  yours    │ machine side ││  │ ● Culture · Ember G. ▸││
│    │    Vashk           │ │           │ Silhouette   ││  ├──────────────────────┤│
...
│    │ + New entity       │                             │                          │
├────┴────────────────────┴─────────────────────────────┴──────────────────────────┤
│ Ashfall · /vol/art/Ashfall.wobu · 812 nodes · ComfyUI connected · queue 0 · ⏱4.2s │ status
└──────────────────────────────────────────────────────────────────────────────────┘
...
| | |
| --- | --- |
| Library | The world tree and the node editor — the screen above. Your default. |
...
breadcrumb** for the selected node. Every crumb is clickable and jumps to that node. On the right,
**Jump to…** opens the command palette, and the gear opens Settings.

## Navigator

Resizable by dragging its right edge, and collapsible from the keyboard.

- **Filter box** at the top — placeholder *Filter world…* — matches node names and one-line
...
- **Drag to re-parent.** Dropping a node onto another moves it in the hierarchy — and therefore
...
- **Status dots.** A node whose description has gone stale gets a small dot; a node someone else has
  open in another session gets a presence dot.
...
> error and offers **Reveal** and **Reload**. It never silently skips a file, and it never writes
> over one it could not read.
...
notes, the **inherits** line listing the layers above this node, the autosave state, and the
...
| Relations | The links this node has and the ones pointing back at it. |

### The Notes split

Two columns. **Left is yours, right is the machine's** — the left is tagged *yours* and the right
*machine side*. That spatial split is the mental model of the whole app: Wobu never writes into your
column, and you can always edit its column. Covered in [Notes and Enhance](notes-and-enhance.md).
...
Each layer card carries a thumbnail or a colour dot, the source node, a count of the text and image
fragments it contributed and how many were dropped, a weight slider, a mute toggle and an **Open
source** button. The full explanation is in [The influence stack](influence.md).
...
> yourself muting the same layer every time, that is a signal to go and fix the node.

## Status bar

Along the bottom in every mode, and mostly read-only: project name and path, whether the folder is a
network share or read-only, sync state and connected peers, other people's sessions, the node count,
which panels are hidden, backend health and the active image model, the job queue depth, the text
model and its context size, and a timer for the running generation.
...
Toasts vanish; some failures should not. Anything that failed — a generation the backend refused, a
save the share rejected, a job that cost money and produced nothing — is kept here with what it
cost, what went wrong in the backend's own words, and where to go next: **Open Settings**, **Open in
Forge** or **Show the entity**. Opening the panel marks everything read; **Clear all** empties it.

## Command palette

From anywhere, and deliberately reachable mid-sentence — it is how you leave where you are. It
searches in two phases: node names and summaries match instantly, then full-text hits from inside
notes and descriptions arrive a moment later, ranked by the index. Results are grouped **Nodes**,
**In notes and descriptions** and **Commands**. Arrow keys move, Enter picks, Escape closes.

The commands are New node, Undo, Redo, Toggle navigator, Toggle inspector and Keyboard shortcuts. On
a read-only folder the ones that write are absent rather than disabled.

## Pickers

Anywhere Wobu has more than a handful of choices — the output preset, an aspect, a relation target,
a parent node, an asset filter — the control is a searchable combobox rather than a dropdown. Type
