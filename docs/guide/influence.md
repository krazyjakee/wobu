...
Given the entity you have selected, Wobu walks outward and collects sources in a fixed order —
outermost first, subject last:

| # | Layer | Source | Contributes |
| --- | --- | --- | --- |
| 1 | Style | The Art Style entity | Medium, rendering, line quality, lighting model, global negatives |
| 2 | World | The World Canon entity | Era, tone, tech and magic level, material vocabulary |
...
| 6 | Subject | the entity itself, plus anything `related_to` | Everything specific |
...
entity's parent is an implicit edge of full weight, so a district inherits its city which inherits
its region. Each source collected becomes one card in the inspector, and the card says *why* it is
there.

## Fragments, not paragraphs

A layer does not contribute a block of prose — it contributes **fragments**. Each fragment knows its
source entity, which description section it came from, its weight, and where it should be routed:
...
4. **Budget.** Trim to the model's usable prompt length, dropping the *lowest-weight* fragments
   ...
   separately and more tightly, per bucket, against the provider's declared limits.
...
its weight, and clicking it opens the entity that produced it.
...
upstream notes. When an image comes out wrong, the tint tells you which entity to go and fix — and
...
| Source entity | With an **Open source** button that jumps to it so you can edit it. |
...
> entity, or change the link.
>
> Link weights, by contrast, *are* part of the world. Set them from the entity's Relations tab when
> a
...
| A reference is not being used | Check its role, and check the provider. `mood` is never sent anywhere, structure roles are inert on providers that do not accept them, and the shipped ComfyUI workflows take no images at all. |
...
Open a result and Wobu shows the recorded stack next to today's, and names the difference: *The
prompt has changed*, or *The parts that can be compared are unchanged*. **Run these settings again**
sends exactly what the receipt holds, without reading today's world at all.
