...
**Rename** on a row changes a scene's display name and nothing else. The file name is minted once at creation and never moves, so a rename does not read as a delete and a create to a sync peer, a watcher or a git history. It is refused while that scene holds an unsaved draft, because it is a whole-document write that would land underneath work you have not saved; save or discard the draft first, or edit **Scene name** in Script. Saving a scene anywhere in the workspace refreshes the library rows behind it, so a new title, a filled slot or a recorded approval is there when you return.
...
No provider call is made by discovery or organization. N5 engine adapters remain excluded. Affected-line tracking (#168) is answered by the [dependency index](35-narrative-dependencies.md) rather than by this projection, and full workspace/Flow acceptance is not implied by this library checkpoint. Supporting-text assets (#167) are authored in the separate [Text library](33-narrative-supporting-text.md) and are not in this index.
