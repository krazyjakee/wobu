...
| Generation and review | Frozen provider requests, immutable results, protected replacement, canonical approval evidence and a paged comparison/bulk Review queue. All six supporting text kinds share generation, review and Release validation, with [native authoring/export evidence](evidence/narrative-supporting/README.md). Precise [dependency tracking](35-narrative-dependencies.md) reports and explains affected lines. | Planning affected rebuilds, localisation/audio and production release workflow. N5 engine integrations are excluded. |
