...
| Deleting a node with children has a stable, documented result. | `Project::delete_node` promotes children to the deleted node's parent and removes backlinks. `project.rs::deleting_a_parent_promotes_its_children` and the undo tests pin that behavior. |
