...
//! capability that today grants exactly what knowing a project ULID and an
//! address grants (nothing on `wobu/sync/1` checks one — see
//! [`wobu_sync::ticket`]). Putting a list that changes whenever a project is
//! shared into a store that prompts on every read, on Linux, would be a dialog
//! per sync poll. When #90 makes a grant mean something, this decision is the
//! one to revisit, and this paragraph is the record of why it was made.
//!
...
//! nobody has open. Keeping the parsed address alone would drop the grant, which
//! is the only field #90 will have to present; keeping the canonical string
...
pub struct Share {
    ...
    /// nothing checks it yet is not a reason to let it drift; see
    /// [`wobu_sync::ticket`].
    pub grant: Grant,
